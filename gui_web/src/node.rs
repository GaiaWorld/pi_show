...
use bevy_ecs::prelude::{Entity, Bundle};
use gui::world::App;
...
use gui::Z_MAX;
use crate::index::{send_create_event, send_delete_event, send_modify_event};

use crate::world::GuiWorld;

fn create<T: Bundle>(world: &mut GuiWorld, t: T) -> Entity {
    let gui = &mut world.gui;
	let entity = gui.spawn().insert_bundle(
		t
	).id();

	gui.idtree_mut().create(entity.id() as usize, entity.generation());
	send_create_event::<Entity>(entity, StyleIndex::Create, gui); // 实体创建事件
	entity
}
...
#[wasm_bindgen]
pub fn create_node(world: u32) -> Entity1 {
	...
	Entity1{id: entity.id(), version: entity.generation()}
}

/// 创建虚拟节点
#[allow(unused_attributes)]
#[wasm_bindgen]
pub fn create_vnode(world: u32) -> Entity1 {
    let world = unsafe { &mut *(world as usize as *mut GuiWorld) };
	let entity = create(world, (
		BorderRadius {
			x: LengthUnit::Pixel(0.0),
	...
	let gui = &mut world.gui;
	gui.get_mut::<NodeState>(entity).unwrap().set_vnode(true);
...
#[allow(unused_attributes)]
#[wasm_bindgen]
pub fn node_is_exist(world: u32, id: u32, generation: u32) -> bool {
	let world = unsafe { &mut *(world as usize as *mut GuiWorld) };
	if let Some(r) = world.gui.com_context.get_mut::<NodeState>(&world.gui, to_entity(id as usize, generation)) {
		if r.is_rnode(){ 
			return true;
		} else {
			return false;
...
#[allow(unused_attributes)]
#[wasm_bindgen]
pub fn create_text_node(world: u32) -> Entity1 {
    let world = unsafe { &mut *(world as usize as *mut GuiWorld) };
    let entity = create(world, (
		BorderRadius {
			x: LengthUnit::Pixel(0.0),
		...
		RectLayoutStyle::default(),
		OtherLayoutStyle::default(),
		TextContent("".to_string(), Atom::from("")),
		StyleMark::default(),
		ZDepth::default(),
		gui::component::calc::Opacity::default(),
		HSV::default(),
...
#[allow(unused_attributes)]
#[wasm_bindgen]
pub fn create_image_node(world: u32) -> Entity1 {
    let world = unsafe { &mut *(world as usize as *mut GuiWorld) };
    let entity = create(world, (
		BorderRadius {
			x: LengthUnit::Pixel(0.0),
...
#[allow(unused_attributes)]
#[wasm_bindgen]
pub fn create_canvas_node(world: u32) -> Entity1 {
    let world = unsafe { &mut *(world as usize as *mut GuiWorld) };
	let entity = create(world, (
		BorderRadius {
			x: LengthUnit::Pixel(0.0),
...
pub fn append_child(world: u32, child: u32, parent: u32) {
	...
	let layer = idtree.insert_child(child as usize, parent as usize, UMAX);

	let node = &idtree[child as usize];
	if layer > 0 {
		send_create_event::<IdTree>(to_entity(child as usize, node.data), StyleIndex::Mount, world);
	} else {
		send_modify_event::<IdTree>(to_entity(child as usize, node.data), StyleIndex::Add, world);
	}
}
...
pub fn insert_before(world: u32, child: u32, brother: u32) {
    ...
    let layer = idtree.insert_brother(
        child as usize,
        brother as usize,
        InsertType::Front,
	);
...
pub fn remove_node(world: u32, node_id: u32) {
    let world = unsafe { &mut *(world as usize as *mut GuiWorld) };
    let world = &mut world.gui;
	let w1 = unsafe{&mut *(world as *const App as usize as *mut App)};
	let mut idtree = w1.idtree_mut();
	// let notify = idtree.get_notify();
	// idtree.remove_with_notify(node_id as usize, &notify);
	
	if let Some(node) = idtree.get(node_id as usize) {
		send_delete_event::<IdTree>(to_entity(node_id as usize, node.data), StyleIndex::UnMount, world);
		idtree.remove_no_notify(node_id as usize);
	}
}
...
#[allow(unused_attributes)]
#[wasm_bindgen]
pub fn destroy_node(world_id: u32, node_id: u32) {
    ...
    let mut idtree = world.idtree_mut();
    // let notify = idtree.get_notify();
    // let nodes = world.node.lend_mut();

	if let Some(node) = idtree.get(node_id as usize) {
		send_delete_event::<IdTree>(to_entity(node_id as usize, node.data), StyleIndex::UnMount, world1);
		
		let head = idtree[node_id as usize].children().head;
		// let h_node = idtree[head]
		world1.despawn(to_entity(node_id as usize, node.data));
		for (id, n) in idtree.recursive_iter(head) {
			world1.despawn(to_entity(id, n.data));
		}
...
pub fn first_child(world: u32, parent: u32) -> usize{
    ...
    let world = &mut world.gui;
	let idtree = world.idtree_mut();
	// let node_state = world.get::<NodeState>();
	let r = idtree[parent as usize].children().head;
	if r < usize::max_value() {
		let head = &idtree[r];
		let entity = to_entity(r, head.data);
		if !world.get::<NodeState>(entity).unwrap().is_rnode() {
...
pub fn set_src(world: u32, node: u32, url: usize) {
    ...
    // let images = world.gui.get_mut::<Image>();
    // images.insert(
    //     node as usize,
    //     Image {
    //         // src: None,
...
#[allow(unused_attributes)]
#[wasm_bindgen]
pub fn offset_top(world: u32, node: u32, version: u32) -> f32 {
    let world = unsafe { &mut *(world as usize as *mut GuiWorld) };
	let world = &mut world.gui;
	if let Some(r) = world.get::<LayoutR>(to_entity(node as usize, version)){
		r.rect.top
	} else {
		0.0
	}
...
#[allow(unused_attributes)]
#[wasm_bindgen]
pub fn offset_left(world: u32, node: u32, version: u32) -> f32 {
    let world = unsafe { &mut *(world as usize as *mut GuiWorld) };
	let world = &mut world.gui;
	if let Some(r) = world.get::<LayoutR>(to_entity(node as usize, version)){
		r.rect.start
...
#[allow(unused_attributes)]
#[wasm_bindgen]
pub fn offset_width(world: u32, node: u32, version: u32) -> f32 {
    let world = unsafe { &mut *(world as usize as *mut GuiWorld) };
	let world = &mut world.gui;
	if let Some(r) = world.get::<LayoutR>(to_entity(node as usize, version)){
...
#[allow(unused_attributes)]
#[wasm_bindgen]
pub fn offset_height(world: u32, node: u32, version: u32) -> f32 {
	// 10.0
    let world = unsafe { &mut *(world as usize as *mut GuiWorld) };
	let world = &mut world.gui;
	if let Some(r) = world.get::<LayoutR>(to_entity(node as usize, version)){
...
//     let world = unsafe { &mut *(world as usize as *mut GuiWorld) };
//     let world = &mut world.gui;
//     let layouts = world.get::<LayoutR>(to_entity(node, version));
...
/// left top width height
#[wasm_bindgen]
pub fn offset_document(world: u32, node_id: usize, version: u32) -> JsValue {
    let node_id = node_id as usize;
    let world = unsafe { &mut *(world as usize as *mut GuiWorld) };
    let world = &mut world.gui;
    // let layouts = world.get::<LayoutR>(to_entity(node, version));
    // let world_matrixs = world.world_matrix.lend();
    // let transforms = world.transform.lend();
    let octs = world.get_resource::<Oct>().unwrap();
...
#[allow(unused_attributes)]
#[wasm_bindgen]
pub fn content_box(world: u32, node: usize, version: u32) -> JsValue {
    let world = unsafe { &mut *(world as usize as *mut GuiWorld) };
    let world = &mut world.gui;
    let idtree = world.idtree();
    let (mut left, mut right, mut top, mut bottom) = (FMAX, 0.0, FMAX, 0.0);
    for (id, n) in idtree.iter(idtree[node as usize].children().head) {
        let l = world.get::<LayoutR>(to_entity(id, n.data)).unwrap();
        let r = l.rect.end;
        let b = l.rect.bottom;
...
pub fn query(world: u32, x: f32, y: f32) -> u32 {
    ...
    let octree = world.get_resource::<Oct>().unwrap();
    let overflow_clip = world.get_resource::<OverflowClip>().unwrap();
	let idtree = world.idtree();
...
fn ab_query_func(arg: &mut AbQueryArgs, _id: usize, aabb: &Aabb2, bind: &usize) {
    ...
    if intersects(&arg.aabb, aabb) {
		...
		let node = match arg.id_tree.get(*bind) {
			Some(r) => r, 
			None => return,
		};
		
		// let enable = arg.world.get::<Enable>( to_entity(*bind, node.data) ).0;
		let z_depth = arg.world.get::<ZDepth>(to_entity(*bind, node.data)).unwrap().0;
        ...
        if z_depth > arg.max_z {
            // let by_overflow = arg.get::<ByOverflow>(*bind).0;
