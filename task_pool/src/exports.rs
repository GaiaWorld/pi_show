...
impl TaskPool {
    ...
    /// 队列长度
    pub fn deque_len(&self, key: f64) -> Option<u32> { self.pool.get_deque(to_key(key)).map(|r| r.deque.len() as u32) }

    /// 释放队列的锁，成功释放，则返回true， 否则返回false
    pub fn deque_unlock(&mut self, key: f64) -> bool { self.pool.deque_unlock(to_key(key)) }

    /// 删除一个任务队列，如果删除成功，返回true， 否则返回false
    pub fn remove_deque(&mut self, key: f64) -> bool { self.pool.remove_deque(to_key(key)) }

    /// 插入一个指定任务权重的并行任务
    pub fn push_deque_task(&mut self, id: f64) -> Option<f64> {
        let id = to_key(id);
        if let Some(deque) = self.pool.get_deque(id) {
            // 这里不使用可变， 是为了让队列状态中的旧长度不更新
            let ptr_usize = deque as *const Deque1<DequeKey, ()> as usize;
            fn ptr(ptr: usize) -> *mut Deque1<DequeKey, ()> { ptr as *mut Deque1<DequeKey, ()> }
    ...
    /// 判断指定时间是否存在任务
    pub fn has_cancel_timer(&self, timeout: f64) -> bool {
        match self.pool.get_cancel_timer().is_null(timeout as u64) {
            Some(b) => !b,
            None => false,
        }
    }
    ...
    /// 判断指定时间内是否存在任务
    pub fn is_cancel_timer_ok(&mut self, timeout: f64) -> bool { self.pool.get_cancel_timer_mut().is_ok(timeout as u64) }

    /// 取到可取消定时器的滚动次数
    pub fn roll_count(&mut self) -> f64 { self.pool.get_cancel_timer_mut().roll_count() as f64 }
...
#[inline]
fn to_key(r: f64) -> DequeKey { DequeKey::from(KeyData::from_ffi(unsafe { transmute(r) })) }
...
#[cfg(test)]
mod test_mod {
    use crate::exports::TaskPool;
    #[test]
    pub fn timer_test() {
        let mut task_pool = TaskPool::new();
        let mut timeout = 6000_f64;
        task_pool.push_cancel_timer(4880302_f64);
        // task_pool.push_cancel_timer(600_f64);
        // task_pool.push_cancel_timer(588_f64);
        // task_pool.push_async(50);
        // println!("==== keys {:?}", task_pool.has_cancel_timer(400_f64));
        println!("==== keys {:?}", task_pool.has_cancel_timer(6010_f64));
        // println!("==== keys {:?}", task_pool.is_cancel_timer_ok(timeout));

        // println!("==== keys {:?}", task_pool.has_cancel_timer(600_f64));
        // println!("==== keys {:?}", task_pool.is_cancel_timer_ok(600_f64));

        // println!("==== keys {:?}", task_pool.is_cancel_timer_ok(400_f64));
    }
}
