...
use bevy_ecs::prelude::{Entity, EventReader, EventWriter, In, Query, Res, ResMut};
...
use crate::util::event::{EntityEvent, EntityEventData, EventType, RenderObjEvent};
...
// 标记用户样式
pub fn userstyle_mark(
	e: In<EntityEventData>,
	mut dirty_list: ResMut<DirtyList>,
	mut style_marks: Query<&mut StyleMark>,
	// mut class_names: Query<&ClassName>,
) {
	mark_local_style_with_dirty(e.0.style_index as usize, e.0.id, &mut dirty_list, &mut style_marks);
}
...
pub fn matrix_mark(
	...
	set_style_dirty(e.0.style_index as usize, e.0.id, &mut dirty_list, &mut style_marks);
}
...
pub fn clear_mark(
	...
	for l in dirty_list.0.iter() {
		if let Ok(mut r) = style_marks.get_mut(*l) {
			r.dirty = [0, 0, 0];
		}
	}
	dirty_list.0.clear();
...
// pub fn single_mark(
// 	e: In<SingleChangeType>,
// 	mut dirty_list: ResMut<DirtyList>,
// 	mut renderobj_events: EventWriter<RenderObjEvent>,
// 	// mut class_names: Query<&ClassName>,
// ) {
// 	match e.0 {
// 		SingleChangeType::FontTexture => {
// 			// renderobj_events.send(RenderObjEvent{
...
// // 标记图片样式(图片比较特殊，涉及到未加载的图片，需要等待)
// pub fn imgstyle_mark<C: HalContext + 'static>(
// 	e: In<UserStyleEvent>,
// 	mut dirty_list: ResMut<DirtyList>,
// 	mut style_marks: Query<&mut StyleMark>,

// 	mut engine: Res<ShareEngine<C>>,
// 	mut img_wait_sheet: Res<ImageWaitSheet>,

// 	mut images: Query<(&mut Image, &mut ImageClip, &mut RectLayoutStyle)>,
// ) {
// 	mark_style_with_dirty(e.index, e.entity, &mut dirty_list, &mut style_marks);
// }

// // 标记图片样式(图片比较特殊，涉及到未加载的图片，需要等待)
// pub fn borderimgstyle_mark<C: HalContext + 'static>(
// 	e: In<UserStyleEvent>,
// 	mut dirty_list: ResMut<DirtyList>,
// 	mut style_marks: Query<&mut StyleMark>,
...
// 	mut images: Query<(&mut BorderImage, &mut BorderImageClip, &mut RectLayoutStyle)>,
// ) {
// 	// mark_style_with_dirty(e.index, e.entity, &mut dirty_list, &mut style_marks);
// }

// // 标记classname样式(图片比较特殊，涉及到未加载的图片，需要等待)
// pub fn classstyle_mark(
// 	e: In<UserStyleEvent>,
// 	mut dirty_list: ResMut<DirtyList>,
// 	mut style_marks: Query<&mut StyleMark>,
// 	mut class_names: Query<&ClassName>,
...
fn set_style_dirty(index: usize, entity: Entity, dirty_list: &mut DirtyList,
	style_marks: &mut Query<&mut StyleMark>) {
	let mut style_mark = match style_marks.get_mut(entity) {
		Ok(r) => r,
		Err(_) => return,
	};
	let offset = index/32;
	let i = index%32;
	set_dirty(dirty_list, entity, offset, i, &mut style_mark);
}


fn mark_local_style_with_dirty(index: usize, entity: Entity, dirty_list: &mut DirtyList,
	style_marks: &mut Query<&mut StyleMark>) {
	let mut style_mark = match style_marks.get_mut(entity) {
		Ok(r) => r,
		Err(_) => return,
	...
	let i = index%32;
	set_dirty(dirty_list, entity, offset, i, &mut style_mark);
	style_mark.local_style[offset] |= 1 << i;
}


#[inline]
fn set_dirty(dirty_list: &mut DirtyList, id: Entity, offset: usize, ty: usize, style_mark: &mut StyleMark) {
	let dirty = &mut style_mark.dirty;
    if dirty[0] == 0 && dirty[1] == 0 && dirty[2] == 0{
...
// #[inline]
// fn set_dirty1(dirty_list: &mut DirtyList, id: Entity, ty: usize, style_mark: &mut StyleMark) {
//     if style_mark.dirty == 0 && style_mark.dirty1 == 0 && style_mark.dirty2 == 0 {
//         dirty_list.0.push(id);
//     }
//     style_mark.dirty1 |= 1<<ty;
// }

// #[inline]
// fn set_dirty2(dirty_list: &mut DirtyList, id: Entity, ty: usize, style_mark: &mut StyleMark) {
//     if style_mark.dirty == 0 && style_mark.dirty1 == 0 && style_mark.dirty2 == 0{
//         dirty_list.0.push(id);
// 	}
//     style_mark.dirty2 |= 1<<ty;
// }
...
//     id: Entity,
// 	mut dirty_list: &mut DirtyList,
// 	mut style_marks: Query<&mut StyleMark>,

// 	mut engine: &mut ShareEngine<C>,
// 	mut img_wait_sheet: &mut ImageWaitSheet,
// 	mut idtree: Res<IdTree>,

// 	mut images: Query<(&mut Image, Option<&mut ImageClip>, &mut RectLayoutStyle)>,
// ) {
// 	if let (Ok(style_mark), Some(_), Ok(image)) = (style_marks.get(id), idtree.get(id.id()), images.get_mut(id)){
