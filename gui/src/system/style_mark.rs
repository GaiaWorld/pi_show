...
use ecs::{CreateEvent, ModifyEvent, DeleteEvent, MultiCaseListener, EntityListener, SingleCaseListener, SingleCaseImpl, MultiCaseImpl, EntityImpl, Runner, idtree::IdTree};
use hal_core::*;
...
impl<'a, L: FlexNode + 'static, C: HalContext + 'static> SingleCaseListener<'a, IdTree, CreateEvent> for StyleMarkSys<L, C>{
	type ReadData = &'a SingleCaseImpl<IdTree>;
	type WriteData = (
		&'a mut MultiCaseImpl<Node, Image>,
		&'a mut MultiCaseImpl<Node, BorderImage>,
...
impl<'a, L: FlexNode + 'static, C: HalContext + 'static> SingleCaseListener<'a, IdTree, DeleteEvent> for StyleMarkSys<L, C>{
	type ReadData = &'a SingleCaseImpl<IdTree>;
	...
	fn listen(&mut self, event: &ModifyEvent, read: Self::ReadData, mut write: Self::WriteData){
		...
		if class_name.one > 0 {
			if old.one != class_name.one {
				set_attr(event.id, class_name.one, &mut self.text_style, read, &mut write);
				if class_name.two > 0 {
					set_attr(event.id, class_name.two, &mut self.text_style, read, &mut write);
				}
				for i in 0..class_name.other.len() {
					set_attr(event.id, class_name.other[i], &mut self.text_style, read, &mut write);
				}
			} else {
...
fn set_attr<L: FlexNode, C: HalContext>(
	...
	set_attr1(id, dirty_list, &class.attrs1, style_mark, text_style, shows, overflows, yoga, obj_fits);
	set_attr2(id, dirty_list, &class.attrs2, style_mark, text_style, yoga, zindexs, opacitys, border_image_repeats, images, border_images, image_wait_sheet, engine, idtree);
...
pub fn set_attr1<L: FlexNode>(
	...
	for layout_attr in layout_attrs.iter() {
		match layout_attr {
			...
			Attribute1::Overflow(r) => if style_mark.local_style1 & StyleType1::Overflow as usize == 0{
	...
	for layout_attr in layout_attrs.iter() {
		match layout_attr {
			...
			Attribute2::ImageUrl(r) => if style_mark.local_style & StyleType::Image as usize == 0 {
				let mut image = Image{src: None, url: r.clone()};
				if let Some(_) = idtree.get(id) {
					set_image(id, StyleType::Image, engine, image_wait_sheet, dirty_list, &mut image, style_mark, ImageType::ImageClass);
				}
				images.insert_no_notify(id, image);
			},
			Attribute2::BorderImageUrl(r) => if style_mark.local_style & StyleType::BorderImage as usize == 0 {
				let mut image = BorderImage(Image{src: None, url: r.clone()});
				if let Some(_) = idtree.get(id) {
					set_image(id, StyleType::BorderImage, engine, image_wait_sheet, dirty_list, &mut image.0, style_mark, ImageType::BorderImageClass);
				}
...
fn set_image<C: HalContext>(
	...
	if image.src.is_none() {
		match engine.texture_res_map.get(&image.url) {
			...
			None => {
				image_wait_sheet.add(&image.url, ImageWait{id: id, ty: wait_ty})
			},
		}
	}
}
...
fn load_image<'a, C: HalContext>(id: usize, write: &mut ImageTextureWrite<'a, C>) {
	...
	if let Some(image) = write.0.get_mut(id) {
		if style_mark.local_style & StyleType::Image as usize != 0 {
			...
			set_image(id, StyleType::Image, &mut *write.4, &mut *write.5, &mut *write.3, image, style_mark, ImageType::ImageClass);
		}
	}
	if let Some(image) = write.1.get_mut(id) {
		if style_mark.local_style & StyleType::BorderImage as usize != 0 {
			...
			set_image(id, StyleType::BorderImage, &mut *write.4, &mut *write.5, &mut *write.3, &mut image.0, style_mark, ImageType::BorderImageClass);
		}
	}
}
...
fn set_image_local<'a, C: HalContext>(id: usize, idtree: &SingleCaseImpl<IdTree>, write: ImageWrite<'a, C>){
	...
	if let Some(_) = idtree.get(id) {
		...
		set_image(id, StyleType::Image, &mut *write.2, &mut *write.3, &mut *write.1, image, style_mark, ImageType::ImageLocal);
	}
}

fn set_border_image_local<'a, C: HalContext>(id: usize, idtree: &SingleCaseImpl<IdTree>, write: BorderImageWrite<'a, C>){
	let style_mark = unsafe {write.0.get_unchecked_mut(id)};
	style_mark.local_style |= StyleType::BorderImage as usize;

	if let Some(_) = idtree.get(id) {
		let image = &mut unsafe {write.4.get_unchecked_mut(id)}.0;
		set_image(id, StyleType::BorderImage, &mut *write.2, &mut *write.3, &mut *write.1, image, style_mark, ImageType::ImageLocal);
	}
}
