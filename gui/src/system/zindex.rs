...
use bevy_ecs::prelude::{Entity, Query, Res, ResMut, In};

// use map::{vecmap::VecMap};
use heap::simple_heap::SimpleHeap;
use dirty::LayerDirty;
use crate::{component::calc::StyleIndex, util::event::ImMessenger};
...
use map::Map;
use crate::{Z_MAX, single::to_entity};
use crate::ROOT;
use crate::single::{IdTree};
use crate::component::{
  user::{ZIndex as ZI},
  calc::{ZDepth, NodeState},
};
use crate::util::vecmap_default::VecMapWithDefault;
use crate::util::event::{EntityEvent, EventType};
use crate::util::util::get_or_default;

pub fn z_system(
	mut z_depth_query: Query<&mut ZDepth>,
	idtree: Res<IdTree>,
	mut local: ResMut<ZIndexImpl>,
	nodestate_query: Query<&NodeState>,
	mut zdepth_event_writer: ImMessenger<EntityEvent<ZDepth>>,
) {

	// 处理zindex的创建、修改，标记脏
	// handle_zi(&zindex_query ,&idtree,&mut zindex_reader, &mut local);

	// 根据脏，计算zindex
	local.calc(&idtree, &mut z_depth_query, &nodestate_query, &mut zdepth_event_writer);
}

pub fn idtree_listen(
	e: In<EntityEvent<IdTree>>,
	nodestate_query: Query<&NodeState>,
	zindex_query: Query<Option<&ZI>>,
	idtree: Res<IdTree>, 
	...
	match e.ty {
		EventType::Delete => {
			let node = &idtree[e.id.id() as usize];
			let parent = node.parent();
			if parent == 0 {
				return;
			}
			let parent = to_entity(parent, idtree[parent].data);
			if let Ok(_entity) = entitys.get(parent) {
				let id = e.id.id() as usize;
				let node = &idtree[id];
				let tail = idtree[parent.id() as usize].children().tail;
				if tail == id {
					let prev = node.prev();
					let parent_id = parent.id() as usize;
					if prev < usize::max_value() {
						local.map[parent_id].empty_min_z = local.map[prev].pre_max_z;
					} else {
						local.map[parent_id].empty_min_z = local.map[parent_id].pre_max_z + 1.0;
					}
				}
			};
		},
		EventType::Create => {
			if let Ok(zindex) = zindex_query.get(e.id) {
				let id = e.id.id() as usize;
				let node = &idtree[id];
				let zindex = get_or_default(zindex, &ZI(0));
				// if let Some(r) = self.map.get_mut(&id) {
				// 	r.pre_min_z = Z_MAX;
				// 	r.pre_max_z = Z_MAX;
				// }

				let zi = &mut local.map[id];
				if zi.old != AUTO {
					// 设置自己成强制脏
					// debug_println!("Recursive==================={}", id);
					zi.dirty = DirtyType::Recursive;
					local.dirty.mark(id, node.layer());
				} else {
					// 设置自己所有非AUTO的子节点为强制脏
					local.recursive_dirty(&nodestate_query, &idtree, node.children().head);
				}

				if zindex.0 == 0 {
					local.set_parent_dirty_width_empty(node, &idtree);
				} else {
					local.set_parent_dirty(node.parent(), &idtree);
				}
			}
...
pub fn entity_listen(
	e: In<EntityEvent<Entity>>,
	query: Query<Option<&ZI>>,
	idtree: Res<IdTree>,
	mut local: ResMut<ZIndexImpl>,
) {
	if let EventType::Create = e.0.ty {
		let id = e.0.id.id() as usize;
		if id == ROOT {
			local.map.insert(id, ZIndex::default());
			let zi = &mut local.map[id];
			...
			local.map.insert(id, ZIndex::default());
			local.modifyz(e.0.id, &idtree, &query);
		}
	}
}

pub fn zindex_listen(
	e: In<EntityEvent<ZI>>,
	query: Query<Option<&ZI>>,
	idtree: Res<IdTree>,
	mut local: ResMut<ZIndexImpl>,
...
impl std::default::Default for ZIndexImpl {
	fn default() -> Self {
		Self::new()
	}
}
...
impl ZIndexImpl {
	...
	fn modifyz(&mut self, entity: Entity, idtree: &Res<IdTree>, zindex: &Query<Option<&ZI>>) {
		if let Ok(z) = zindex.get(entity) {
			let z = get_or_default(z, &ZI(0)).0;
	...
	fn calc(
		&mut self, 
		idtree: &IdTree, 
		zdepth: &mut Query<&mut ZDepth>, 
		node_states: &Query<&NodeState>,
		zdepth_event_writer: &mut ImMessenger<EntityEvent<ZDepth>>,) {
		...
		for (id, layer) in self.dirty.iter() {
			...
			zdepth.get_mut(entity).unwrap().0 = min_z;
			zdepth_event_writer.send(EntityEvent::new_modify(entity, StyleIndex::ZDepth));
			...
			let max_z_z = match dirty {
				DirtyType::Normal => {
					...
					self.cache.calc(&mut self.map, idtree, zdepth, min_z, max_z, node.count(), node_states, zdepth_event_writer)
				},
				DirtyType::Recursive => {
					self.cache.sort(zdepth, node_states, &self.map, idtree, node.children().head, 0);
					debug_println!("recursive_calc, id:{:?}, min_z:{:?}, max_z: {:?}", id, min_z, max_z);
					self.cache.recursive_calc(zdepth_event_writer, node_states, &mut self.map, idtree, zdepth, min_z, max_z, node.count())
				},
				DirtyType::Empty => { // 
					if self.cache.node_heap.len() == 0 {
						...
						if (count as f32) <= empty_z && empty_z <= max_z - min_z - 1.{ // 如果是空
							self.cache.calc_empty(&mut self.map, idtree, zdepth, old_empty_min_z, max_z, count, arr, node_states, zdepth_event_writer)
						} else {
	...
	// 设置自己所有非AUTO的子节点为强制脏
	fn recursive_dirty(&mut self, node_states: &Query<&NodeState>, idtree: &IdTree, child: usize) {
		for (id, n) in idtree.iter(child) {
			let entity = to_entity(id, n.data);
			if !node_states.get(entity).unwrap().is_rnode() {
				continue;
			}

			let zi = &mut self.map[child];
			if zi.old == -1 {
				self.recursive_dirty(node_states, idtree, n.children().head);
			}else {
				zi.dirty = DirtyType::Recursive;
				self.dirty.mark(id, n.layer());
			}
		}
	}
}
...
impl Cache {
	...
	// 循环计算子节点， 分类排序
	fn sort(&mut self, zdepth: &mut Query<&mut ZDepth>, node_states: &Query<&NodeState>, map: &VecMapWithDefault<ZIndex>, idtree: &IdTree, child: usize, mut order: usize) -> usize {
	...
	// 循环计算子节点， 分类排序
	fn sort_width_empty(&mut self, zdepth: &mut Query<&mut ZDepth>, node_states: &Query<&NodeState>, map: &VecMapWithDefault<ZIndex>, idtree: &IdTree, child: usize, mut order: usize, empty_min_z: f32) -> (usize, Vec<ZSort>) {
	...
	// 计算真正的z
	fn calc(&mut self, map: &mut VecMapWithDefault<ZIndex>, idtree: &IdTree, zdepth: &mut Query<&mut ZDepth>, mut min_z: f32, mut max_z: f32, count: usize, node_states: &Query<&NodeState>, zdepth_event_writer: &mut ImMessenger<EntityEvent<ZDepth>>) -> f32 {
		...
		while let Some(ZSort(_, _, n_id, c)) = self.negative_heap.pop() {
			...
			adjust(zdepth_event_writer, node_states, map, idtree, zdepth, n_id, &idtree[n_id], min_z, max_z, f32::NAN, 0.);
			min_z = max_z;
		}
		debug_println!("z_auto: len: {:?}, value: {:?}", self.z_auto.len(), self.z_auto);
		for n_id in &self.z_auto {
			adjust(zdepth_event_writer, node_states, map, idtree, zdepth, *n_id, &idtree[*n_id], min_z, min_z, f32::NAN, 0.);
			min_z += 1.;
		}
		self.z_auto.clear();
	...
	fn calc_empty(&mut self, map: &mut VecMapWithDefault<ZIndex>, idtree: &IdTree, zdepth: &mut Query<&mut ZDepth>, mut min_z: f32, mut max_z: f32, count: usize, arr: Vec<ZSort>, node_states: &Query<&NodeState>, zdepth_event_writer: &mut ImMessenger<EntityEvent<ZDepth>>) -> f32 {
		min_z += 1.;
	...
	fn recursive_calc(
		...
		zdepth_event_writer: &mut ImMessenger<EntityEvent<ZDepth>>,
		node_states: &Query<&NodeState>, 
		map: &mut VecMapWithDefault<ZIndex>, 
		idtree: &IdTree, 
		zdepth: &mut Query<&mut ZDepth>, 
		mut min_z: f32, 
		mut max_z: f32, 
		...
		while start < self.temp.len() {
			...
			debug_println!("recursive_calc, id:{:?}, min_z:{:?}, max_z: {:?}", id, min_z, max_z);
			map[id].empty_min_z = self.recursive_calc(zdepth_event_writer, node_states, map, idtree, zdepth, min_z, max_z, node.count());
		}
		max_z
	}
...
fn adjust(zdepth_event_writer: &mut ImMessenger<EntityEvent<ZDepth>>,node_states: &Query<&NodeState>, map: &mut VecMapWithDefault<ZIndex>, idtree: &IdTree, zdepth: &mut Query<&mut ZDepth>, id: usize, node: &IdNode<u32>, min_z: f32, max_z: f32, rate: f32, parent_min: f32) {
	let entity = to_entity(id, node.data);
	if !node_states.get(entity).unwrap().is_rnode() {
		return;
	}
	...
	for (i, n) in idtree.iter(node.children().head) {
		adjust(zdepth_event_writer, node_states, map, idtree, zdepth, i, n, min, 0., r, old_min);
	}
}
