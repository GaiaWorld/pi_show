...
use component::render::{SdfDefines, RenderObj, TextDefines, ImageDefines, DefinesId, DefinesList};
...
impl CreateProgram {
    pub fn init(component_mgr: &mut GuiComponentMgr) -> Rc<CreateProgram>{
        let r = Rc::new(CreateProgram(RefCell::new(CreateProgramImpl::new())));
        component_mgr.render_obj._group.register_create_handler(Rc::downgrade(&(r.clone() as Rc<ComponentHandler<RenderObj, CreateEvent, GuiComponentMgr>>)));
        component_mgr.render_obj._group.register_delete_handler(Rc::downgrade(&(r.clone() as Rc<ComponentHandler<RenderObj, DeleteEvent, GuiComponentMgr>>)));
        component_mgr.render_obj.defines.sdf._group.register_modify_field_handler(Rc::downgrade(&(r.clone() as Rc<ComponentHandler<SdfDefines, ModifyFieldEvent, GuiComponentMgr>>)));
        component_mgr.render_obj.defines.text._group.register_modify_field_handler(Rc::downgrade(&(r.clone() as Rc<ComponentHandler<TextDefines, ModifyFieldEvent, GuiComponentMgr>>)));
        component_mgr.render_obj.defines.image._group.register_modify_field_handler(Rc::downgrade(&(r.clone() as Rc<ComponentHandler<ImageDefines, ModifyFieldEvent, GuiComponentMgr>>)));
        r
    }
}
 
impl ComponentHandler<RenderObj, CreateEvent, GuiComponentMgr> for CreateProgram{
...
impl ComponentHandler<RenderObj, DeleteEvent, GuiComponentMgr> for CreateProgram{
...
impl ComponentHandler<SdfDefines, ModifyFieldEvent, GuiComponentMgr> for CreateProgram{
...
impl ComponentHandler<TextDefines, ModifyFieldEvent, GuiComponentMgr> for CreateProgram{
    fn handle(&self, event: &ModifyFieldEvent, _component_mgr: &mut GuiComponentMgr){
        let ModifyFieldEvent{id: _, parent, field: _} = event;
        self.0.borrow_mut().marked_dirty(*parent);
    }
...
impl ComponentHandler<ImageDefines, ModifyFieldEvent, GuiComponentMgr> for CreateProgram{
    fn handle(&self, event: &ModifyFieldEvent, _component_mgr: &mut GuiComponentMgr){
        let ModifyFieldEvent{id: _, parent, field: _} = event;
        self.0.borrow_mut().marked_dirty(*parent);
    }
...
//创建program
impl System<(), GuiComponentMgr> for CreateProgram{
    fn run(&self, _e: &(), component_mgr: &mut GuiComponentMgr){
        ...
        for render_obj_id in borrow_mut.dirtys.iter() {
            ...
            match program {
                Ok(v) => {
                    let mgr_ptr = component_mgr as *const GuiComponentMgr as usize;
