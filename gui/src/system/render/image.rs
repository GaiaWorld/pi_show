		...
		for id in dirty_list.0.iter() {
			...
			if dirty & GEO_DIRTY != 0 {
				...
				if dirty & StyleType::Image as usize != 0 {
					...
					render_obj.paramter.set_texture("texture", (&image.src.as_ref().unwrap().bind, &self.default_sampler));
				}
...
fn get_pos_uv(img: &Image, clip: Option<&ImageClip>, fit: Option<&ObjectFit>, layout: &Layout) -> (Aabb2, Aabb2){
	...
	let (size, mut uv1, mut uv2) = match clip {
		Some(c) => {
			let size = Vector2::new(src.width as f32 * (c.max.x - c.min.x).abs(), src.height as f32 * (c.max.y - c.min.y).abs());
			(size, c.min, c.max)
		},
		_ => (Vector2::new(src.width as f32, src.height as f32), Point2::new(0.0,0.0), Point2::new(1.0,1.0))
	};
