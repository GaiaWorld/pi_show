...
//     // LocalSrc = 0x100,
//     // LocalImageClip =  0x200,
//     // LocalObjectFit =  0x400,
// }
...
//         let (layouts, border_radiuss, z_depths, images, image_clips, object_fits, world_matrixs, transforms, opacitys, unit_quad, default_table) = read;
//         let (render_objs, engine) = write;
//         let geo_dirty = DrityType::ObjectFit as usize    |
//                         DrityType::ImageClip as usize    | 
//                         DrityType::Layout as usize       |
//                         DrityType::BorderRadius as usize | 
//                         DrityType::Src as usize;
...
//                 // src修改， 修改texture
//                 if dirty & DrityType::Src as usize != 0 {
//                     // 如果四边形与图片宽高一样， 使用点采样， TODO
//                     render_obj.paramter.set_texture("texture", (image.src.bind.clone(), self.default_sampler.clone()));
//                 }

//                 notify.modify_event(item.index, "geometry", 0);
//                 notify.modify_event(item.index, "ubo", 0);
//                 dirty &= !(DrityType::Matrix as usize); // 已经计算了世界矩阵， 设置世界矩阵不脏
//             }

//             // 世界矩阵脏， 设置世界矩阵ubo
//             if dirty & DrityType::Matrix as usize != 0 {
...
//             if dirty & DrityType::Opacity as usize != 0 || dirty & DrityType::Src as usize != 0 {
...
//                 notify.modify_event(item.index, "is_opacity", 0);
//             }
//         }
...
//         Some(r) => r,
//         None => {
//             let mut uvs = [uv.min.x, uv.min.y, uv.min.x, uv.max.y, uv.max.x, uv.max.y, uv.max.x, uv.min.y];
...
// // 插入渲染对象
// impl<'a, C: HalContext + 'static> MultiCaseListener<'a, Node, Image, CreateEvent> for ImageSys<C>{
//     type ReadData = (
//         &'a MultiCaseImpl<Node, ZDepth>,
...
//         let (z_depths, default_state) = read;
...
//         self.items.create(event.id, index);
//     }
// }
