...
impl<'a, C: Context> Runner<'a> for ImageSys<C>{
    type ReadData = (
...
// 插入渲染对象
impl<'a, C: Context> MultiCaseListener<'a, Node, Image<C>, CreateEvent> for ImageSys<C>{
    type ReadData = (
        &'a MultiCaseImpl<Node, Image<C>>,
        &'a MultiCaseImpl<Node, BorderRadius>,
        &'a MultiCaseImpl<Node, ZDepth>,
...
// 修改渲染对象
impl<'a, C: Context> MultiCaseListener<'a, Node, Image<C>, ModifyEvent> for ImageSys<C>{
...
impl<'a, C: Context> MultiCaseListener<'a, Node, ImageClip, ModifyEvent> for ImageSys<C>{
    type ReadData = ();
    type WriteData = ();
...
impl<'a, C: Context> MultiCaseListener<'a, Node, ObjectFit, ModifyEvent> for ImageSys<C>{
    type ReadData = ();
    type WriteData = ();
    fn listen(&mut self, event: &ModifyEvent, _: Self::ReadData, _: Self::WriteData){
        if let Some(item) = self.render_map.get_mut(event.id) {
...
impl<'a, C: Context> MultiCaseListener<'a, Node, ImageClip, CreateEvent> for ImageSys<C>{
    type ReadData = ();
    type WriteData = ();
...
impl<'a, C: Context> MultiCaseListener<'a, Node, ObjectFit, CreateEvent> for ImageSys<C>{
    type ReadData = ();
    type WriteData = ();
    fn listen(&mut self, event: &CreateEvent, _: Self::ReadData, _: Self::WriteData){
        if let Some(item) = self.render_map.get_mut(event.id) {
...
// 删除渲染对象
impl<'a, C: Context> MultiCaseListener<'a, Node, Image<C>, DeleteEvent> for ImageSys<C>{
    type ReadData = ();
    type WriteData = &'a mut SingleCaseImpl<RenderObjs<C>>;
...
//布局修改， 需要重新计算顶点
impl<'a, C: Context> MultiCaseListener<'a, Node, Layout, ModifyEvent> for ImageSys<C>{
    type ReadData = ();
    type WriteData = ();
...
impl<'a, C: Context> MultiCaseListener<'a, Node, Opacity, ModifyEvent> for ImageSys<C>{
    type ReadData = (&'a MultiCaseImpl<Node, Opacity>, &'a MultiCaseImpl<Node, Image<C>>);
    type WriteData = &'a mut SingleCaseImpl<RenderObjs<C>>;
...
impl<'a, C: Context> MultiCaseListener<'a, Node, WorldMatrixRender, ModifyEvent> for ImageSys<C>{
...
impl<'a, C: Context> MultiCaseListener<'a, Node, WorldMatrixRender, CreateEvent> for ImageSys<C>{
    type ReadData = MatrixRead<'a>;
    type WriteData = &'a mut SingleCaseImpl<RenderObjs<C>>;
...
fn get_geo_flow<C: Context>(radius: &BorderRadius, layout: &Layout, z_depth: f32, image: &Image<C>, image_clip: Option<&ImageClip>, object_fit: Option<&ObjectFit>) -> (Vec<f32>, Vec<f32>, Vec<u16>) {
...
unsafe impl<C: Context> Sync for ImageSys<C>{}
unsafe impl<C: Context> Send for ImageSys<C>{}

impl_system!{
    ImageSys<C> where [C: Context],
    true,
    {
