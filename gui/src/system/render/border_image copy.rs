...
impl<'a> Runner<'a> for BorderImageSys{
    ...
    fn setup(&mut self, _: Self::ReadData, write: Self::WriteData){
        ...
        match engine.res_mgr.get::<SamplerRes>(&hash) {
            Some(r) => self.default_sampler = Some(r.clone()),
            None => {
                let res = SamplerRes::new(hash, engine.gl.create_sampler(Share::new(s)).unwrap());
                self.default_sampler = Some(engine.res_mgr.create::<SamplerRes>(res));
            }
        };
    }
}

impl<'a> MultiCaseListener<'a, Node, BorderImageClip, CreateEvent> for BorderImageSys{
    type ReadData = ();
    type WriteData = ();
...
impl<'a> MultiCaseListener<'a, Node, BorderImageClip, ModifyEvent> for BorderImageSys{
    type ReadData = ();
    type WriteData = ();
...
impl<'a> MultiCaseListener<'a, Node, BorderImageSlice, CreateEvent> for BorderImageSys{
    type ReadData = ();
    type WriteData = ();
    fn listen(&mut self, event: &CreateEvent, _: Self::ReadData, _: Self::WriteData){
        if let Some(item) = self.render_map.get_mut(event.id) {
...
impl<'a> MultiCaseListener<'a, Node, BorderImageSlice, ModifyEvent> for BorderImageSys{
    type ReadData = ();
    type WriteData = ();
    fn listen(&mut self, event: &ModifyEvent, _: Self::ReadData, _: Self::WriteData){
        if let Some(item) = self.render_map.get_mut(event.id) {
...
impl<'a> MultiCaseListener<'a, Node, BorderImageRepeat, CreateEvent> for BorderImageSys{
    type ReadData = ();
    type WriteData = ();
    fn listen(&mut self, event: &CreateEvent, _: Self::ReadData, _: Self::WriteData){
        if let Some(item) = self.render_map.get_mut(event.id) {
...
impl<'a> MultiCaseListener<'a, Node, BorderImageRepeat, ModifyEvent> for BorderImageSys{
    type ReadData = ();
    type WriteData = ();
    fn listen(&mut self, event: &ModifyEvent, _: Self::ReadData, _: Self::WriteData){
        if let Some(item) = self.render_map.get_mut(event.id) {
...
// 插入渲染对象
impl<'a> MultiCaseListener<'a, Node, BorderImage, CreateEvent> for BorderImageSys{
    type ReadData = (
        &'a MultiCaseImpl<Node, BorderImage>,
        &'a MultiCaseImpl<Node, ZDepth>,
        &'a MultiCaseImpl<Node, Layout>,
    ...
    fn listen(&mut self, event: &CreateEvent, read: Self::ReadData, write: Self::WriteData){
        ...
        let mut ubos: FnvHashMap<Atom, Share<Uniforms>> = FnvHashMap::default();
        let defines = Vec::new();
        ...
        let render_obj: RenderObj = RenderObj {
            depth: z_depth - 0.1,
            depth_diff: -0.1,
            visibility: false,
...
// 修改渲染对象
impl<'a> MultiCaseListener<'a, Node, BorderImage, ModifyEvent> for BorderImageSys{
    type ReadData = (&'a MultiCaseImpl<Node, Opacity>, &'a MultiCaseImpl<Node, BorderImage>);
...
// 删除渲染对象
impl<'a> MultiCaseListener<'a, Node, BorderImage, DeleteEvent> for BorderImageSys{
    type ReadData = ();
    type WriteData = &'a mut SingleCaseImpl<RenderObjs>;
...
//布局修改， 需要重新计算顶点
impl<'a> MultiCaseListener<'a, Node, Layout, ModifyEvent> for BorderImageSys{
    type ReadData = ();
    type WriteData = ();
...
impl<'a> MultiCaseListener<'a, Node, Opacity, ModifyEvent> for BorderImageSys{
    type ReadData = (&'a MultiCaseImpl<Node, Opacity>, &'a MultiCaseImpl<Node, BorderImage>);
    type WriteData = &'a mut SingleCaseImpl<RenderObjs>;
...
impl<'a> MultiCaseListener<'a, Node, WorldMatrixRender, ModifyEvent> for BorderImageSys{
...
impl<'a> MultiCaseListener<'a, Node, WorldMatrixRender, CreateEvent> for BorderImageSys{
    type ReadData = MatrixRead<'a>;
    type WriteData = &'a mut SingleCaseImpl<RenderObjs>;
...
impl<'a> BorderImageSys {
    fn change_is_opacity(&mut self, _id: usize, opacity: f32, image: &BorderImage, index: usize, render_objs: &mut SingleCaseImpl<RenderObjs>){
        let is_opacity = if opacity < 1.0 {
            false
        }else if let ROpacity::Opaque = image.src.opacity{
            true
...
unsafe impl Sync for BorderImageSys{}
unsafe impl Send for BorderImageSys{}


pub fn get_border_image_stream<'a + 'static > (
  img: &BorderImage,
...
impl_system!{
    BorderImageSys where [C: Context],
    true,
    {
