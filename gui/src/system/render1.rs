...
fn render<C: Context + Share>(gl: &mut C, obj: &RenderObj<C>){
    gl.set_pipeline(&mut (obj.pipeline.clone() as Arc<AsRef<Pipeline>>));
    // gl.draw(&(obj.geometry.clone() as Arc<AsRef<<C as Context>::ContextGeometry>>), &(obj.ubos.clone() as Arc<AsRef<Uniforms<C>>>));
}
...
impl<'a, C: Context + Share> PartialOrd for OpacityOrd<'a, C> {
	fn partial_cmp(&self, other: &OpacityOrd<'a, C>) -> Option<Ordering> {
        (self.0.pipeline.as_ref() as *const Pipeline as usize).partial_cmp(&(other.0.pipeline.as_ref() as *const Pipeline as usize))
	}
}

impl<'a, C: Context + Share> PartialEq for OpacityOrd<'a, C>{
	 fn eq(&self, other: &OpacityOrd<'a, C>) -> bool {
        (self.0.pipeline.as_ref() as *const Pipeline as usize).eq(&(other.0.pipeline.as_ref() as *const Pipeline as usize))
    }
}
