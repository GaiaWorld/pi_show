...
//插入Layout 和 L 组件
impl<'a, L: FlexNode + 'static> EntityListener<'a, Node, CreateEvent> for LayoutSys<L>{
    type ReadData = ();
...
impl<'a, L: FlexNode + 'static> SingleCaseListener<'a, IdTree, ModifyEvent> for LayoutSys<L>{
...
impl<'a, L: FlexNode + 'static> SingleCaseListener<'a, IdTree, CreateEvent> for LayoutSys<L>{
    type ReadData = (&'a SingleCaseImpl<IdTree>, &'a MultiCaseImpl<Node, L>);
    type WriteData = ();
...
impl<'a, L: FlexNode + 'static> SingleCaseListener<'a, IdTree, DeleteEvent> for LayoutSys<L>{
    type ReadData = (&'a SingleCaseImpl<IdTree>, &'a MultiCaseImpl<Node, L>);
    type WriteData = ();
...
fn add_yoga<L: FlexNode + 'static>(id: usize, idtree: &SingleCaseImpl<IdTree>, yogas: &MultiCaseImpl<Node, L>){
...
impl_system!{
    LayoutSys<L> where [L: FlexNode + 'static],
    false,
    {
