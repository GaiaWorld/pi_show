...
impl<'a, L: FlexNode + 'static> MultiCaseListener<'a, Node, TextContent, CreateEvent>
    for LayoutImpl<L>
{
    type ReadData = ();
...
// 监听CharBlock的删除
impl<'a, L: FlexNode + 'static> MultiCaseListener<'a, Node, CharBlock<L>, DeleteEvent>
    for LayoutImpl<L>
{
    type ReadData = ();
    type WriteData = &'a mut MultiCaseImpl<Node, CharBlock<L>>;
...
extern "C" fn text_callback<L: FlexNode + 'static>(
    node: L,
    mut width: f32,
    _width_mode: YGMeasureMode,
    _height: f32,
    _height_mode: YGMeasureMode,
) -> YGSize {
...
fn set_gylph<'a, L: FlexNode + 'static>(id: usize, read: &Read<L>, write: &mut Write<L>) {
    ...
    for char_node in cb.chars.iter_mut() {
        if char_node.ch > ' ' {
            char_node.ch_id_or_count = write.2.calc_gylph(
                &tex_font,
                cb.font_size as usize,
                cb.stroke_width as usize,
                weight,
                scale,
...
fn calc<'a, L: FlexNode + 'static>(
    ...
    cb.font_height = tex_font
        .0
        .get_font_height(cb.font_size as usize, text_style.text.stroke.width);
    if cb.font_size == 0.0 {
        #[cfg(feature = "warning")]
    ...
    let mut tex_param = TexParam {
        cb: cb,
        tex_font: &tex_font,
        text_style: text_style,
        word_margin: text_style.text.letter_spacing / 2.0 - text_style.text.stroke.width,
    };
...
fn update_char<L: FlexNode + 'static>(
    id: usize,
    tex_param: &mut TexParam<L>,
    c: char,
    w: f32,
    sw: usize,
    font: &mut FontSheet,
...
fn calc_text<'a, L: FlexNode + 'static>(
    tex_param: &mut TexParam<L>,
    text: &'a str,
    sw: usize,
    font: &mut FontSheet,
    ...
    for cr in split(
        text,
        true,
        tex_param.text_style.text.white_space.preserve_spaces(),
    ) {
        match cr {
            SplitResult::Newline => {
...
// 更新字符，如果字符不同，则清空后重新插入
fn update_char1<L: FlexNode + 'static>(
    tex_param: &mut TexParam<L>,
    c: char,
    w: f32,
    sw: usize,
...
// 设置节点的宽高
fn set_node1<L: FlexNode + 'static>(
    tex_param: &mut TexParam<L>,
    c: char,
    w: f32,
    sw: usize,
    ...
    if c > ' ' {
        let r = font.measure(
            tex_param.tex_font,
            tex_param.cb.font_size as usize,
            sw,
            tex_param.text_style.font.weight,
            c,
        );
...
fn wrap_line<L: FlexNode + 'static>(
    cb: &mut CharBlock<L>,
...
fn align_line<L: FlexNode + 'static>(
    cb: &mut CharBlock<L>,
    (end, mut start, _, line_width): (usize, usize, usize, f32),
    limit_width: f32,
    x_fix: f32,
    y_fix: f32,
    get_x_fix: fn(f32, f32) -> f32,
) {
...
fn justify_line<L: FlexNode + 'static>(
    cb: &mut CharBlock<L>,
    (end, mut start, word_count, line_width): (usize, usize, usize, f32),
    limit_width: f32,
    x_fix: f32,
    y_fix: f32,
) {
