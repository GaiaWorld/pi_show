...
use component::render::{Bind, Defines, DefinesId, RenderObj, SdfDefines, SdfDefinesWriteRef};
use component::style::color::Color;
use component::style::element::{Rect, ElementId, RectWriteRef};
...
impl RectChange{
    pub fn init(component_mgr: &mut GuiComponentMgr) -> Rc<RectChange> {
        let r = RectChange{
...
impl System<(), GuiComponentMgr> for RectChange {
...
impl ShapeDirty {
    pub fn init(component_mgr: &mut GuiComponentMgr) -> Rc<ShapeDirty> {
        let r = Rc::new(ShapeDirty);
        component_mgr.node.z_depth.register_handler(Rc::downgrade(&(r.clone() as Rc<ComponentHandler<Node, ModifyFieldEvent, GuiComponentMgr>>),));
        component_mgr.node.bound_box._group.register_modify_field_handler(Rc::downgrade(&(r.clone() as Rc<ComponentHandler<MathAabb3, ModifyFieldEvent, GuiComponentMgr>>),));
        r
    }
}

impl ComponentHandler<MathAabb3, ModifyFieldEvent, GuiComponentMgr> for ShapeDirty {
...
impl ComponentHandler<Node, ModifyFieldEvent, GuiComponentMgr> for ShapeDirty {
    fn handle(&self, event: &ModifyFieldEvent, component_mgr: &mut GuiComponentMgr) {
...
//param(id: node_id)
fn mark_rect_shape_dirty(id: usize, mgr: &mut GuiComponentMgr){
...
impl RectSet {
    pub fn init(component_mgr: &mut GuiComponentMgr) -> Rc<RectSet> {
        ...
        component_mgr
            ...
            .register_create_handler(Rc::downgrade(
                &(r.clone() as Rc<ComponentHandler<Rect, CreateEvent, GuiComponentMgr>>),
            ));
        component_mgr
            .node
            .element
            ...
            .register_delete_handler(Rc::downgrade(
                &(r.clone() as Rc<ComponentHandler<Rect, DeleteEvent, GuiComponentMgr>>),
            ));
        r
    }
}

impl System<(), GuiComponentMgr> for RectSet {
    fn run(&self, _e: &(), _component_mgr: &mut GuiComponentMgr) {}
}

//监听Rect的创建， 创建对应的program
impl ComponentHandler<Rect, CreateEvent, GuiComponentMgr> for RectSet {
...
impl ComponentHandler<Rect, DeleteEvent, GuiComponentMgr> for RectSet {
...
impl ColorSet {
    pub fn init(component_mgr: &mut GuiComponentMgr) -> Rc<ColorSet> {
        ...
        component_mgr
            ...
            ._group
            .register_create_handler(Rc::downgrade(
                &(r.clone() as Rc<ComponentHandler<Color, CreateEvent, GuiComponentMgr>>),
            ));
        component_mgr
            .node
            .element
            ...
            .register_modify_field_handler(Rc::downgrade(
                &(r.clone() as Rc<ComponentHandler<Color, ModifyFieldEvent, GuiComponentMgr>>),
            ));
        component_mgr
            .node
            .element
            ...
            ._group
            .register_delete_handler(Rc::downgrade(
                &(r.clone() as Rc<ComponentHandler<Color, DeleteEvent, GuiComponentMgr>>),
            ));
        r
    }
}
    ...
    fn handle(&self, event: &CreateEvent, component_mgr: &mut GuiComponentMgr) {
            ...
            component_mgr.render_obj._group.get(render_obj_id).defines
        {
            modify_color_defines(
                defines_id,
                &unsafe { &mut *(component_mgr as *mut GuiComponentMgr) }
                    .node
                    .element
                    .rect
                    .color
...
impl System<(), GuiComponentMgr> for ColorSet {
    fn run(&self, _e: &(), _component_mgr: &mut GuiComponentMgr) {}
}
...
fn modify_color_defines(defines_id: usize, color: &Color, mgr: &mut GuiComponentMgr) {
    let mut defines_ref =
        SdfDefinesWriteRef::new(defines_id, mgr.render_obj.defines.to_usize(), unsafe {
            &mut *(mgr as *mut GuiComponentMgr)
        });
    match color {
...
impl RadiusSet {
    pub fn init(component_mgr: &mut GuiComponentMgr) -> Rc<RadiusSet> {
        ...
        component_mgr
            ...
            ._group
            .register_modify_field_handler(Rc::downgrade(
                &(r.clone() as Rc<ComponentHandler<Rect, ModifyFieldEvent, GuiComponentMgr>>),
            ));
        r
    }
}
...
impl RectBind {
    pub fn new(id: usize, mgr: &mut GuiComponentMgr) -> RectBind {
...
impl Bind for RectBind {
    unsafe fn init_locations(&self, context: usize, renderobj_id: usize) {
        let mgr = &mut *(context as *mut GuiComponentMgr);
