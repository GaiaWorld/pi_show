...
use ecs::{
SingleCaseListener,
system::{MultiCaseListener, EntityListener, Runner, },
...
impl<'a> MultiCaseListener<'a, Node, Overflow, DeleteEvent> for OverflowImpl {
	...
	fn listen(&mut self, event: &DeleteEvent, overflows: Self::ReadData, overflow_clip: Self::WriteData) {
...
// impl<'a> MultiCaseListener<'a, Node, Visibility, ModifyEvent> for OverflowImpl{
// 	type ReadData = Read<'a>;
// 	type WriteData = Write<'a>;

// 	fn listen(&mut self, event: &ModifyEvent, read: Self::ReadData, mut write: Self::WriteData) {
// 		let visibility = unsafe { read.9.get_unchecked(event.id) };
// 		let overflow = match read.1.get(event.id){Some(r) => **r, _ => false};
// 		if overflow && !visibility.0 {
// 			let notify = write.0.get_notify();
...
// 			if unsafe{  read.1.get_unchecked(node.parent)}.0 {
// 				let i = get_index(write.0, node.parent);
// 				if i > 0 {
// 					by = add_index(by, 1<<(i - 1));
// 				}
// 			}
// 			// println!("by: {}, id: {}", by, event.id);
// 			unsafe { write.1.get_unchecked_write(event.id) }.modify(|r|{
// 				r.0 = by;
// 				return true;
// 			});
// 			// write.1.get_unchecked_write(event.id)}.set_0(by);

// 			if overflow {
// 				// 添加根上的overflow的裁剪矩形
// 				if get_index(write.0, event.id) == 0 {
// 					// println!("create_clip: {}", event.id);
// 					create_clip(event.id, write.0);
// 					self.mark_dirty(event.id, StyleType1::Overflow as usize, node.layer,  &mut write.3);
...
impl<'a> SingleCaseListener<'a, IdTree, DeleteEvent> for OverflowImpl {
	...
	fn listen(&mut self, event: &DeleteEvent, read: Self::ReadData, write: Self::WriteData) {
		let node = unsafe{ read.0.get_unchecked(event.id)};
		let notify = write.0.get_notify();
		if match read.1.get(event.id){Some(r) => **r, _ => false} {
			remove_index(&mut *write.0, event.id, &notify);
		}
...
// fn set_by_overflow(id: usize, by: usize, node: &ecs::idtree::Node, read: &Read, write: &mut Write) {
// 	unsafe {write.1.get_unchecked_write(id)}.set_0(by);
// 	let overflow = match read.1.get(id){Some(r) => **r, _ => false};
// 	if overflow {
// 		let visibility = unsafe { read.9.get_unchecked(id) }.0;
// 		if visibility {
// 			// 添加根上的overflow的裁剪矩形
// 			// let i = set_index(&mut *write.0, 0, id);
...
// 	for (id, _n) in read.0.iter(node.children.head) {
// 		let node = unsafe{ read.0.get_unchecked(id)};
// 		set_by_overflow(id, by, node, read, write);
// 	}
// 	// let overflow = match read.1.get(id){Some(r) => **r, _ => false};
// 	// if overflow {
// 	// 	return;
