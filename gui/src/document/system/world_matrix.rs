...
use document::component::style::transform::{Transform};
...
impl WorldMatrix {
    pub fn init(component_mgr: &mut DocumentMgr) -> Rc<WorldMatrix>{
        let system = Rc::new(WorldMatrix(RefCell::new(WorldMatrixImpl::new())));
        component_mgr.node.transform._group.register_modify_field_handler(Rc::downgrade(&(system.clone() as Rc<ComponentHandler<Transform, ModifyFieldEvent, DocumentMgr>>)));
        component_mgr.node.transform._group.register_delete_handler(Rc::downgrade(&(system.clone() as Rc<ComponentHandler<Transform, DeleteEvent, DocumentMgr>>)));
        component_mgr.node.transform._group.register_create_handler(Rc::downgrade(&(system.clone() as Rc<ComponentHandler<Transform, CreateEvent, DocumentMgr>>)));
        component_mgr.node.position._group.register_modify_field_handler(Rc::downgrade(&(system.clone() as Rc<ComponentHandler<Vector3, ModifyFieldEvent, DocumentMgr>>)));
        component_mgr.node._group.register_create_handler(Rc::downgrade(&(system.clone() as Rc<ComponentHandler<Node, CreateEvent, DocumentMgr>>)));
        system
    }
}

impl ComponentHandler<Transform, ModifyFieldEvent, DocumentMgr> for WorldMatrix{
...
impl ComponentHandler<Transform, DeleteEvent, DocumentMgr> for WorldMatrix{
...
impl ComponentHandler<Transform, CreateEvent, DocumentMgr> for WorldMatrix{
...
impl ComponentHandler<Node, CreateEvent, DocumentMgr> for WorldMatrix{
...
impl ComponentHandler<Vector3, ModifyFieldEvent, DocumentMgr> for WorldMatrix{
    fn handle(&self, event: &ModifyFieldEvent, component_mgr: &mut DocumentMgr){
        let ModifyFieldEvent{id: _, parent, field: _} = event;
        self.0.borrow_mut().marked_dirty(*parent, component_mgr);
    }
...
impl System<(), DocumentMgr> for WorldMatrix{
...
impl WorldMatrixImpl {
    ...
    //计算世界矩阵
    pub fn cal_matrix(&mut self, component_mgr: &mut DocumentMgr){
    ...
    pub fn marked_dirty(&mut self, node_id: usize, mgr: &mut DocumentMgr){
        {
    ...
    pub fn delete_dirty(&mut self, node_id: usize, mgr: &mut DocumentMgr){
        let dirty_mark = *unsafe{self.dirty_mark_list.get_unchecked_mut(node_id)};
        if dirty_mark == true {
            let layer = mgr.node._group.get_mut(node_id).layer;
...
//计算世界矩阵
fn modify_matrix(dirty_mark_list: &mut VecMap<bool>, node_id: usize, component_mgr: &mut DocumentMgr) {
...
mod test{
    ...
    use document::component::style::transform::Transform;
    use generic_component::math::{Vector3};
    use document::system::world_matrix::WorldMatrix;
    ...
    fn new_world() -> World<DocumentMgr, ()>{
        let mut world: World<DocumentMgr, ()> = World::new(DocumentMgr::new());
        let systems: Vec<Rc<System<(), DocumentMgr>>> = vec![WorldMatrix::init(&mut world.component_mgr)];
