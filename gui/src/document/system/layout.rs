...
use document::component::node::{RectSize, YogaContex, Node};
...
impl Layout {
    pub fn init(component_mgr: &mut DocumentMgr) -> Rc<Layout>{
        let system = Rc::new(Layout(RefCell::new(LayoutImpl::new())));
        component_mgr.node.layout_change.register_handler(Rc::downgrade(&(system.clone() as Rc<ComponentHandler<Node, ModifyFieldEvent, DocumentMgr>>)));
...
impl ComponentHandler<Node, ModifyFieldEvent, DocumentMgr> for Layout{
...
//监听node的删除事件，以便删除脏
impl ComponentHandler<Node, DeleteEvent, DocumentMgr> for Layout{
...
impl System<(), DocumentMgr> for Layout{
    fn run(&self, _e: &(), component_mgr: &mut DocumentMgr){
        let root_id = component_mgr.root_id;
        let width = component_mgr.object2d.component_mgr.width;
        let height = component_mgr.object2d.component_mgr.height;
...
impl LayoutImpl {
    ...
    pub fn update_layout(&mut self, mgr: &mut DocumentMgr){
...
extern "C" fn callback(context: *const c_void) {
    ...
    let component_mgr = unsafe{ &mut *(yoga_context.mgr as *mut DocumentMgr) };
