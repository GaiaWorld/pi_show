...
impl OpacitySys {
    pub fn init(component_mgr: &mut DocumentMgr) -> Rc<OpacitySys>{
        let system = Rc::new(OpacitySys(RefCell::new(OpacitySysImpl::new())));
        component_mgr.node.opacity.register_handler(Rc::downgrade(&(system.clone() as Rc<ComponentHandler<Node, ModifyFieldEvent, DocumentMgr>>)));
        ...
        component_mgr.node._group.register_delete_handler(Rc::downgrade(&(system.clone() as Rc<ComponentHandler<Node, DeleteEvent, DocumentMgr>>)));
        system
    }
}

//监听opacity属性的改变
impl ComponentHandler<Node, ModifyFieldEvent, DocumentMgr> for OpacitySys{
...
//监听Node的创建， 设置脏标志
impl ComponentHandler<Node, CreateEvent, DocumentMgr> for OpacitySys{
...
//监听Node的删除创建， 删除脏标志
impl ComponentHandler<Node, DeleteEvent, DocumentMgr> for OpacitySys{
...
impl System<(), DocumentMgr> for OpacitySys{
...
impl OpacitySysImpl {
    ...
    //计算不透明度
    pub fn cal_opacity(&mut self, component_mgr: &mut DocumentMgr){
...
fn modify_opacity(dirty_mark_list: &mut VecMap<bool>, parent_real_opacity: f32, node_id: usize, component_mgr: &mut DocumentMgr) {
...
mod test {
    ...
    use document::system::opacity::OpacitySys;
    ...
    fn new_world() -> World<DocumentMgr, ()>{
        ...
        let systems: Vec<Rc<System<(), DocumentMgr>>> = vec![OpacitySys::init(&mut world.component_mgr)];
