...
use document::component::node::{Node, RectSize};
use generic_component::math::{Matrix4, Vector3, Point2};
use object2d::{Overflow};

pub struct OverflowSys();

impl OverflowSys {
  pub fn init(mgr: &mut DocumentMgr) -> Rc<OverflowSys> {
    let rc = Rc::new(OverflowSys());
    mgr.node.overflow.register_handler(Rc::downgrade(&(rc.clone() as Rc<ComponentHandler<Node, ModifyFieldEvent, DocumentMgr>>)));
    ...
    mgr.node.world_matrix._group.register_modify_field_handler(Rc::downgrade(&(rc.clone() as Rc<ComponentHandler<Matrix4, ModifyFieldEvent, DocumentMgr>>)));
    rc
  }
}

//监听overflow属性的改变
impl ComponentHandler<Node, ModifyFieldEvent, DocumentMgr> for OverflowSys {
  fn handle(&self, event: &ModifyFieldEvent, mgr: &mut DocumentMgr) {
    let ModifyFieldEvent{id, parent: _, field: _} = event;
    let (index, by, child) = {
      let node = mgr.node._group.get_mut(*id);
      if node.overflow {
        // 添加根上的overflow的裁剪矩形
        let i = set_index(&mut mgr.object2d.component_mgr.overflow, 0, *id);
        if i == 0 {
          return;
        }
        ...
        // 删除根上的overflow的裁剪矩形
        let i = set_index(&mut mgr.object2d.component_mgr.overflow, *id, 0);
        if i == 0 {
          return;
        }
        (i, node.by_overflow & !i, node.get_childs_mut().get_first())
    ...
    mgr.object2d.component_mgr.overflow.handlers.clone().notify(SingleModifyEvent{field:""}, &mut mgr.object2d.component_mgr);
  }
}
//监听了Matrix组件的修改
impl ComponentHandler<Matrix4, ModifyFieldEvent, DocumentMgr> for OverflowSys{
    fn handle(&self, event: &ModifyFieldEvent, mgr: &mut DocumentMgr){
        let ModifyFieldEvent{id, parent, field: _} = event;
        let node = mgr.node._group.get(*parent);
        if node.overflow {
          let i = get_index(&mgr.object2d.component_mgr.overflow, *parent);
          if i > 0 {
            ...
            mgr.object2d.component_mgr.overflow.1[i] = calc_point(pos, size, world_matrix);
            mgr.object2d.component_mgr.overflow.handlers.clone().notify(SingleModifyEvent{field:""}, &mut mgr.object2d.component_mgr);
          }
        }
    }
...
//监听Node的创建， 设置脏标志
impl ComponentHandler<Node, CreateEvent, DocumentMgr> for OverflowSys {
  fn handle(&self, event: &CreateEvent, mgr: &mut DocumentMgr) {
...
//监听Node的删除创建， 删除脏标志
impl ComponentHandler<Node, DeleteEvent, DocumentMgr> for OverflowSys {
  fn handle(&self, event: &DeleteEvent, mgr: &mut DocumentMgr) {
    let DeleteEvent{id, parent: _} = event;
    // 检查该节点是否有overflow
    if mgr.node._group.get_mut(*id).overflow {
      // 删除根上的overflow的裁剪矩形
      if set_index(&mut mgr.object2d.component_mgr.overflow, *id, 0) > 0 {
        mgr.object2d.component_mgr.overflow.handlers.clone().notify(SingleModifyEvent{field:""}, &mut mgr.object2d.component_mgr);
      }
    }
  }
...
#[inline]
fn adjust(mgr: &mut DocumentMgr, mut child: usize, index: usize, ops: fn(a:usize, b:usize)->usize) {
  //递归计算子节点的z
  loop {
    if child == 0 {
        return;
...
fn test(){
    ...
    let systems: Vec<Rc<System<(), DocumentMgr>>> = vec![];
...
#[cfg(not(feature = "web"))]
#[cfg(test)]
fn test_world_overflow(world: &mut World<DocumentMgr, ()>){
    ...
    println!("modify run-----------------------------------------");
    world.run(());
    println!("ooo:{:?}", world.component_mgr.object2d.component_mgr.overflow.deref());
...
#[cfg(not(feature = "web"))]
#[cfg(test)]
fn print_node(mgr: &DocumentMgr, id: usize) {
