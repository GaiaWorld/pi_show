...
// use document::component::style::border::Border;
// use document::component::style::element::{Rect, ElementId};
// use generic_component::color::{ColorId};
// use generic_component::math::{Color as CgColor};
// use world::{DocumentMgr};
// use generic_component::math::{ Aabb3 };
// use render::vector_sdf::Index;

// pub struct Sdf(RefCell<SdfRenderImpl>);

// impl Sdf {
//     pub fn init(component_mgr: &mut DocumentMgr) -> Rc<Sdf>{
//         let system = Rc::new(Sdf(RefCell::new(SdfRenderImpl::new())));
//         component_mgr.node.element.rect._group.register_handler(Rc::downgrade(&(system.clone() as Rc<ComponentHandler<Rect, DocumentMgr>>)));
//         component_mgr.node.border._group.register_handler(Rc::downgrade(&(system.clone() as Rc<ComponentHandler<Border, DocumentMgr>>)));
//         component_mgr.node.bound_box._group.register_handler(Rc::downgrade(&(system.clone() as Rc<ComponentHandler<Aabb3, DocumentMgr>>)));
//         system
//     }
// }

// impl ComponentHandler<Rect, DocumentMgr> for Sdf{
//     fn handle(&self, event: &Event, component_mgr: &mut DocumentMgr){
//         match event {
...
// // impl ComponentHandler<CircleElem, DocumentMgr> for Sdf{
// //     fn handle(&self, event: &Event, component_mgr: &mut DocumentMgr){
// //         match event {
// //             Event::Create{id: _, parent} => {
...
// impl ComponentHandler<Aabb3, DocumentMgr> for Sdf{
...
//                 js!{
//                     console.log("aabb3---------------------modify start1");
//                 }
...
// // impl ComponentHandler<Circle, DocumentMgr> for Sdf{
// //     fn handle(&self, event: &Event, _component_mgr: &mut DocumentMgr){
// //         match event {
// //             //此处只监听了Rect的radius字段的改变， Rect的其他字段对渲染没有意义， 最终的各点坐标还与布局有关（布局影响的包围盒）， 
// //             Event::ModifyField{id: _, parent, field} => {
...
// impl ComponentHandler<Border, DocumentMgr> for Sdf{
//     fn handle(&self, event: &Event, _component_mgr: &mut DocumentMgr){
//         match event {
...
// // impl ComponentHandler<SdfStyle, DocumentMgr> for Sdf{
// //     fn handle(&self, event: &Event, _component_mgr: &mut DocumentMgr){
// //         match event {
// //             Event::Create{id: _, parent} => self.0.borrow_mut().marked_style_dirty(*parent),
// //             Event::ModifyField{id: _, parent, field: _} => self.0.borrow_mut().marked_style_dirty(*parent),
...
// impl System<(), DocumentMgr> for Sdf{
...
//                         },
//                         ColorId::RGBA(ref c) => {
//                             unsafe {component_mgr.opaque_vector.update_style(border, component_mgr.node.element.rect.color.rgba._group.get(*c), index.index.tex) };
//                         },
//                         _ => {
...
//     rect_dirtys: Slab<Dirty>,
//     rect_dirtys_list: Vec<usize>,
...
//             rect_dirtys: Slab::new(),
//             rect_dirtys_list: Vec::new(),
//         }
//     }
...
//     fn alloc_from_sdf(&mut self, node_id: usize, component_mgr: &mut DocumentMgr ){
//         let index = component_mgr.opaque_vector.alloc(); // 分配buffer
//         self.index_map_reverse.insert(index, node_id); // 对index的attribute建立反向映射
//         self.indexs.insert(node_id, IndexExtent{index: index, shape_dirty: false, style_dirty: false}); // 保存所有分配的buffer的index
//     }

//     fn free_from_sdf(&mut self, node_id: usize, component_mgr: &mut DocumentMgr ){
...
// // fn add_rect(r_point: &RectPoint, s_point: &SdfStylePoint, component_mgr: &mut DocumentMgr){
