...
use document::{Z_MAX, DocumentMgr};
use document::component::node::{Node};

pub struct ZIndexSys(RefCell<ZIndexImpl>);

impl ZIndexSys {
  pub fn init(mgr: &mut DocumentMgr) -> Rc<ZIndexSys> {
    let rc = Rc::new(ZIndexSys(RefCell::new(ZIndexImpl::new())));
    mgr.node.zindex.register_handler(Rc::downgrade(&(rc.clone() as Rc<ComponentHandler<Node, ModifyFieldEvent, DocumentMgr>>)));
...
//监听zindex属性的改变
impl ComponentHandler<Node, ModifyFieldEvent, DocumentMgr> for ZIndexSys {
...
//监听Node的创建， 设置脏标志
impl ComponentHandler<Node, CreateEvent, DocumentMgr> for ZIndexSys {
...
//监听Node的删除创建， 删除脏标志
impl ComponentHandler<Node, DeleteEvent, DocumentMgr> for ZIndexSys {
...
impl System<(), DocumentMgr> for ZIndexSys {
  fn run(&self, _e: &(), mgr: &mut DocumentMgr) {
...
impl ZIndexImpl {
  ...
  // 设置节点对应堆叠上下文的节点脏
  fn set_dirty(&mut self, mut node_id: usize, mgr: &mut DocumentMgr) {
    while node_id > 0 {
  ...
  fn delete_dirty(&mut self, node_id: usize, mgr: &mut DocumentMgr) {
    let zi = unsafe {self.links.get_unchecked_mut(node_id)};
    if zi.dirty {
  ...
  // 整理方法
  fn calc(&mut self, mgr: &mut DocumentMgr) {
...
impl Cache {
  ...
  // 循环计算子节点， 分类排序
  fn sort(&mut self, mgr: &mut DocumentMgr, mut child: usize, mut order: usize) -> usize {
  ...
  // 计算真正的z
  fn calc(&mut self, links: &mut VecMap<ZIndex>, mgr: &mut DocumentMgr, mut min_z: f32, mut max_z: f32, count: usize) {
...
fn adjust(links: &mut VecMap<ZIndex>, mgr: &mut DocumentMgr, node_id: usize, min_z: f32, max_z: f32, rate: f32, parent_min: f32) {
...
fn test(){
    ...
    let systems: Vec<Rc<System<(), DocumentMgr>>> = vec![zz.clone()];
...
#[cfg(not(feature = "web"))]
#[cfg(test)]
fn test_world_z(world: &mut World<DocumentMgr, ()>, zz: Rc<ZIndexSys>){
...
#[cfg(not(feature = "web"))]
#[cfg(test)]
fn print_node(mgr: &DocumentMgr, zz: Rc<ZIndexSys>, id: usize) {
