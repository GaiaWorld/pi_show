...
impl Render {
    pub fn init(_component_mgr: &mut DocumentMgr) -> Rc<Render>{
        let system = Rc::new(Render(RefCell::new(RenderImpl::new())));
        // component_mgr.render_obj._group.register_create_handler(Rc::downgrade(&(system.clone() as Rc<ComponentHandler<RenderObj, CreateEvent, DocumentMgr>>)));
        // component_mgr.render_obj._group.register_delete_handler(Rc::downgrade(&(system.clone() as Rc<ComponentHandler<RenderObj, DeleteEvent, DocumentMgr>>)));
        // component_mgr.render_obj.is_opaque.register_handler(Rc::downgrade(&(system.clone() as Rc<ComponentHandler<RenderObj, ModifyFieldEvent, DocumentMgr>>)));
        // component_mgr.render_obj.z_index.register_handler(Rc::downgrade(&(system.clone() as Rc<ComponentHandler<RenderObj, ModifyFieldEvent, DocumentMgr>>)));
        system
    }
}
...
impl RenderImpl {
    ...
    pub fn render(&mut self, mgr: &mut DocumentMgr) {
        self.list_obj(mgr);
        let mgr_ptr = mgr as *const DocumentMgr as usize;
    ...
    //对不透明物体和透明物体排序
    fn list_obj(&mut self, mgr: &mut DocumentMgr){
