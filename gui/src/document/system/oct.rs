...
use document::component::node::{RectSize};
use document::DocumentMgr;
use generic_component::math::{Matrix4, Aabb3 as C_Aabb3};
use cg::{Aabb3, Vector4, Point3};

pub struct Oct(RefCell<OctImpl>);

impl Oct {
    pub fn init(component_mgr: &mut DocumentMgr) -> Rc<Oct>{
        let r = Rc::new(Oct(RefCell::new(OctImpl::new())));
        component_mgr.node.extent._group.register_modify_field_handler(Rc::downgrade(&(r.clone() as Rc<ComponentHandler<RectSize, ModifyFieldEvent, DocumentMgr>>)));
        component_mgr.node.extent._group.register_delete_handler(Rc::downgrade(&(r.clone() as Rc<ComponentHandler<RectSize, DeleteEvent, DocumentMgr>>)));
        component_mgr.node.extent._group.register_create_handler(Rc::downgrade(&(r.clone() as Rc<ComponentHandler<RectSize, CreateEvent, DocumentMgr>>)));
        component_mgr.node.world_matrix._group.register_modify_field_handler(Rc::downgrade(&(r.clone() as Rc<ComponentHandler<Matrix4, ModifyFieldEvent, DocumentMgr>>)));
        r
    }
}

impl ComponentHandler<RectSize, ModifyFieldEvent, DocumentMgr> for Oct{
...
impl ComponentHandler<RectSize, CreateEvent, DocumentMgr> for Oct{
...
impl ComponentHandler<RectSize, DeleteEvent, DocumentMgr> for Oct{
...
//监听了Matrix组件的修改
impl ComponentHandler<Matrix4, ModifyFieldEvent, DocumentMgr> for Oct{
    fn handle(&self, event: &ModifyFieldEvent, _component_mgr: &mut DocumentMgr){
        let ModifyFieldEvent{id: _, parent, field: _} = event;
        self.0.borrow_mut().marked_dirty(*parent);
    }
...
impl System<(), DocumentMgr> for Oct{
...
impl OctImpl {
    ...
    //计算包围盒
    pub fn cal_bound_box(&mut self, mgr: &mut DocumentMgr){
    ...
    pub fn add_aabb(&mut self, node_id: usize, mgr: &mut DocumentMgr){
    ...
    pub fn remove_aabb(&mut self, node_id: usize, mgr: &mut DocumentMgr){
        let node = mgr.node._group.get_mut(node_id);
        if node.bound_box > 0 {
...
mod test {
    ...
    fn new_world() -> World<DocumentMgr, ()>{
        ...
        let systems: Vec<Rc<System<(), DocumentMgr>>> = vec![Oct::init(&mut world.component_mgr)];
