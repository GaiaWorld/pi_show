...
pub fn create_world<L: FlexNode, C: HalContext + 'static>(
	...
	let positions = engine.create_buffer_res(POSITIONUNIT.get_hash() as u64, BufferType::Attribute, 8, Some(BufferData::Float(&[
		0.0, 0.0,
		0.0, 1.0,
		1.0, 1.0,
		1.0, 0.0
	])), false);
	let indices = engine.create_buffer_res(INDEXUNIT.get_hash() as u64, BufferType::Indices, 6, Some(BufferData::Short(&[
		0, 1, 2, 0, 2, 3
	])), false);
