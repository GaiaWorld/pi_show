...
// use world_doc::component::style::element::{Rect, ElementId};
// use component::color::{ColorId};
...
// impl Sdf {
//     pub fn init(component_mgr: &mut WorldDocMgr) -> Rc<Sdf>{
//         let system = Rc::new(Sdf(RefCell::new(SdfRenderImpl::new())));
//         component_mgr.node.element.rect._group.register_handler(Rc::downgrade(&(system.clone() as Rc<ComponentHandler<Rect, WorldDocMgr>>)));
//         component_mgr.node.border._group.register_handler(Rc::downgrade(&(system.clone() as Rc<ComponentHandler<Border, WorldDocMgr>>)));
//         component_mgr.node.bound_box._group.register_handler(Rc::downgrade(&(system.clone() as Rc<ComponentHandler<Aabb3, WorldDocMgr>>)));
//         system
//     }
// }

// impl ComponentHandler<Rect, WorldDocMgr> for Sdf{
//     fn handle(&self, event: &Event, component_mgr: &mut WorldDocMgr){
//         match event {
...
// // impl ComponentHandler<CircleElem, WorldDocMgr> for Sdf{
// //     fn handle(&self, event: &Event, component_mgr: &mut WorldDocMgr){
// //         match event {
// //             Event::Create{id: _, parent} => {
...
// impl ComponentHandler<Aabb3, WorldDocMgr> for Sdf{
...
// // impl ComponentHandler<Circle, WorldDocMgr> for Sdf{
// //     fn handle(&self, event: &Event, _component_mgr: &mut WorldDocMgr){
// //         match event {
// //             //此处只监听了Rect的radius字段的改变， Rect的其他字段对渲染没有意义， 最终的各点坐标还与布局有关（布局影响的包围盒）， 
// //             Event::ModifyField{id: _, parent, field} => {
...
// impl ComponentHandler<Border, WorldDocMgr> for Sdf{
//     fn handle(&self, event: &Event, _component_mgr: &mut WorldDocMgr){
//         match event {
...
// // impl ComponentHandler<SdfStyle, WorldDocMgr> for Sdf{
// //     fn handle(&self, event: &Event, _component_mgr: &mut WorldDocMgr){
// //         match event {
// //             Event::Create{id: _, parent} => self.0.borrow_mut().marked_style_dirty(*parent),
// //             Event::ModifyField{id: _, parent, field: _} => self.0.borrow_mut().marked_style_dirty(*parent),
...
// impl System<(), WorldDocMgr> for Sdf{
...
//     fn alloc_from_sdf(&mut self, node_id: usize, component_mgr: &mut WorldDocMgr ){
...
//     fn free_from_sdf(&mut self, node_id: usize, component_mgr: &mut WorldDocMgr ){
...
// // fn add_rect(r_point: &RectPoint, s_point: &SdfStylePoint, component_mgr: &mut WorldDocMgr){
