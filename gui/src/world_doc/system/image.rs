...
// //监听background_color修改事件， 修改sdf2d上对应的值
// impl ComponentHandler<Color, ModifyFieldEvent, WorldDocMgr> for ImageSys {
...
// impl ComponentHandler<Color, CreateEvent, WorldDocMgr> for ImageSys {
//     fn handle(&self, event: &CreateEvent, component_mgr: &mut WorldDocMgr) {
//         let CreateEvent { id, parent } = event; 
...
// impl ComponentHandler<Color, DeleteEvent, WorldDocMgr> for ImageSys {
...
// impl ComponentHandler<MathColor, CreateEvent, WorldDocMgr> for ImageSys {
//     fn handle(&self, event: &CreateEvent, component_mgr: &mut WorldDocMgr) {
//         let CreateEvent { id, parent } = event; 
//         let mut borrow_mut = self.0.borrow_mut();
//         let node_id =  component_mgr.node.decorate._group.get(*parent).parent;
...
// impl ComponentHandler<MathColor, DeleteEvent, WorldDocMgr> for ImageSys {
//     fn handle(&self, event: &DeleteEvent, component_mgr: &mut WorldDocMgr) {
//         let DeleteEvent { id: _, parent } = event; 
//         let border_color_id = component_mgr.node.decorate._group.get(*parent).border_color;
//         if border_color_id == 0 {
...
// //监听border_color的删除事件， 尝试删除对应的sdf2d组件
// impl ComponentHandler<MathColor, ModifyFieldEvent, WorldDocMgr> for ImageSys {
//     fn handle(&self, event: &ModifyFieldEvent, component_mgr: &mut WorldDocMgr) {
//         let ModifyFieldEvent { id: _, parent, field: _ } = event; 
//         let sdf_id = *(unsafe { self.0.borrow_mut().color_sdf2d_map.get_unchecked(*parent) });
...
// impl ComponentHandler<BoxShadow, CreateEvent, WorldDocMgr> for ImageSys {
//     fn handle(&self, event: &CreateEvent, component_mgr: &mut WorldDocMgr) {
...
// impl ComponentHandler<BoxShadow, DeleteEvent, WorldDocMgr> for ImageSys {
...
// impl ComponentHandler<BoxShadow, ModifyFieldEvent, WorldDocMgr> for ImageSys {
//     fn handle(&self, event: &ModifyFieldEvent, component_mgr: &mut WorldDocMgr) {
...
// impl ComponentHandler<Aabb3, ModifyFieldEvent, WorldDocMgr> for ImageSys {
//     fn handle(&self, event: &ModifyFieldEvent, component_mgr: &mut WorldDocMgr) {
