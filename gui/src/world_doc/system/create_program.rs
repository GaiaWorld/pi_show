...
use world_doc::component::render::{SdfDefines, RenderObj, TextDefines, ImageDefines, DefinesId, DefinesList};
...
impl CreateProgram {
    pub fn init(component_mgr: &mut WorldDocMgr) -> Rc<CreateProgram>{
        let r = Rc::new(CreateProgram(RefCell::new(CreateProgramImpl::new())));
        component_mgr.render_obj._group.register_create_handler(Rc::downgrade(&(r.clone() as Rc<ComponentHandler<RenderObj, CreateEvent, WorldDocMgr>>)));
        component_mgr.render_obj._group.register_delete_handler(Rc::downgrade(&(r.clone() as Rc<ComponentHandler<RenderObj, DeleteEvent, WorldDocMgr>>)));
        component_mgr.render_obj.defines.sdf._group.register_modify_field_handler(Rc::downgrade(&(r.clone() as Rc<ComponentHandler<SdfDefines, ModifyFieldEvent, WorldDocMgr>>)));
        component_mgr.render_obj.defines.text._group.register_modify_field_handler(Rc::downgrade(&(r.clone() as Rc<ComponentHandler<TextDefines, ModifyFieldEvent, WorldDocMgr>>)));
        component_mgr.render_obj.defines.image._group.register_modify_field_handler(Rc::downgrade(&(r.clone() as Rc<ComponentHandler<ImageDefines, ModifyFieldEvent, WorldDocMgr>>)));
        r
    }
}
 
impl ComponentHandler<RenderObj, CreateEvent, WorldDocMgr> for CreateProgram{
...
impl ComponentHandler<RenderObj, DeleteEvent, WorldDocMgr> for CreateProgram{
...
impl ComponentHandler<SdfDefines, ModifyFieldEvent, WorldDocMgr> for CreateProgram{
...
impl ComponentHandler<TextDefines, ModifyFieldEvent, WorldDocMgr> for CreateProgram{
    fn handle(&self, event: &ModifyFieldEvent, _component_mgr: &mut WorldDocMgr){
        let ModifyFieldEvent{id: _, parent, field: _} = event;
        self.0.borrow_mut().marked_dirty(*parent);
    }
...
impl ComponentHandler<ImageDefines, ModifyFieldEvent, WorldDocMgr> for CreateProgram{
    fn handle(&self, event: &ModifyFieldEvent, _component_mgr: &mut WorldDocMgr){
        let ModifyFieldEvent{id: _, parent, field: _} = event;
        self.0.borrow_mut().marked_dirty(*parent);
    }
...
//创建program
impl System<(), WorldDocMgr> for CreateProgram{
    fn run(&self, _e: &(), component_mgr: &mut WorldDocMgr){
        ...
        for render_obj_id in borrow_mut.dirtys.iter() {
            ...
            match program {
                Ok(v) => {
                    let mgr_ptr = component_mgr as *const WorldDocMgr as usize;
