...
use world_doc::component::node::{RectSize, YogaContex, Node};
...
impl Layout {
    pub fn init(component_mgr: &mut WorldDocMgr) -> Rc<Layout>{
        let system = Rc::new(Layout(RefCell::new(LayoutImpl::new())));
        component_mgr.node.layout_change.register_handler(Rc::downgrade(&(system.clone() as Rc<ComponentHandler<Node, ModifyFieldEvent, WorldDocMgr>>)));
...
//监听node的删除事件，以便删除脏
impl ComponentHandler<Node, DeleteEvent, WorldDocMgr> for Layout{
...
impl LayoutImpl {
    ...
    pub fn update_layout(&mut self, mgr: &mut WorldDocMgr){
