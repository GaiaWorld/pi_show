...
use component::math::{Color as MathColor, Aabb3 as MathAabb3};
use world_doc::component::render::{Bind, Defines, DefinesId, RenderObj, SdfDefines, SdfDefinesWriteRef};
use component::color::Color;
use world_doc::component::style::element::{Rect, ElementId, RectWriteRef};
...
impl RectChange{
    pub fn init(component_mgr: &mut WorldDocMgr) -> Rc<RectChange> {
        let r = RectChange{
...
impl System<(), WorldDocMgr> for RectChange {
...
impl ShapeDirty {
    pub fn init(component_mgr: &mut WorldDocMgr) -> Rc<ShapeDirty> {
        ...
        component_mgr.node.bound_box._group.register_modify_field_handler(Rc::downgrade(&(r.clone() as Rc<ComponentHandler<MathAabb3, ModifyFieldEvent, WorldDocMgr>>),));
        r
    }
}

impl ComponentHandler<MathAabb3, ModifyFieldEvent, WorldDocMgr> for ShapeDirty {
...
impl ComponentHandler<Node, ModifyFieldEvent, WorldDocMgr> for ShapeDirty {
    fn handle(&self, event: &ModifyFieldEvent, component_mgr: &mut WorldDocMgr) {
...
//param(id: node_id)
fn mark_rect_shape_dirty(id: usize, mgr: &mut WorldDocMgr){
...
impl RectSet {
    pub fn init(component_mgr: &mut WorldDocMgr) -> Rc<RectSet> {
        ...
        component_mgr
            ...
            .register_create_handler(Rc::downgrade(
                &(r.clone() as Rc<ComponentHandler<Rect, CreateEvent, WorldDocMgr>>),
            ));
        component_mgr
            .node
            .element
            ...
            .register_delete_handler(Rc::downgrade(
                &(r.clone() as Rc<ComponentHandler<Rect, DeleteEvent, WorldDocMgr>>),
            ));
        r
    }
}

impl System<(), WorldDocMgr> for RectSet {
    fn run(&self, _e: &(), _component_mgr: &mut WorldDocMgr) {}
}

//监听Rect的创建， 创建对应的program
impl ComponentHandler<Rect, CreateEvent, WorldDocMgr> for RectSet {
...
impl ComponentHandler<Rect, DeleteEvent, WorldDocMgr> for RectSet {
...
impl ColorSet {
    pub fn init(component_mgr: &mut WorldDocMgr) -> Rc<ColorSet> {
        ...
        component_mgr
            ...
            ._group
            .register_create_handler(Rc::downgrade(
                &(r.clone() as Rc<ComponentHandler<Color, CreateEvent, WorldDocMgr>>),
            ));
        component_mgr
            .node
            .element
            ...
            .register_modify_field_handler(Rc::downgrade(
                &(r.clone() as Rc<ComponentHandler<Color, ModifyFieldEvent, WorldDocMgr>>),
            ));
        component_mgr
            .node
            .element
            ...
            ._group
            .register_delete_handler(Rc::downgrade(
                &(r.clone() as Rc<ComponentHandler<Color, DeleteEvent, WorldDocMgr>>),
            ));
        r
    }
}

impl ComponentHandler<Color, CreateEvent, WorldDocMgr> for ColorSet {
    fn handle(&self, event: &CreateEvent, component_mgr: &mut WorldDocMgr) {
        let CreateEvent { id, parent } = event;
            ...
            component_mgr.render_obj._group.get(render_obj_id).defines
        {
            modify_color_defines(
                defines_id,
                &unsafe { &mut *(component_mgr as *mut WorldDocMgr) }
                    .node
                    .element
                    .rect
                    .color
...
impl ComponentHandler<Color, ModifyFieldEvent, WorldDocMgr> for ColorSet {
    fn handle(&self, event: &ModifyFieldEvent, component_mgr: &mut WorldDocMgr) {
...
impl ComponentHandler<Color, DeleteEvent, WorldDocMgr> for ColorSet {
    fn handle(&self, event: &DeleteEvent, component_mgr: &mut WorldDocMgr) {
...
impl System<(), WorldDocMgr> for ColorSet {
    fn run(&self, _e: &(), _component_mgr: &mut WorldDocMgr) {}
}

fn modify_color_defines(defines_id: usize, color: &Color, mgr: &mut WorldDocMgr) {
    let mut defines_ref =
        SdfDefinesWriteRef::new(defines_id, mgr.render_obj.defines.to_usize(), unsafe {
            &mut *(mgr as *mut WorldDocMgr)
        });
    match color {
...
impl BorderColorSet {
    pub fn init(component_mgr: &mut WorldDocMgr) -> Rc<BorderColorSet> {
...
impl System<(), WorldDocMgr> for BorderColorSet {
    fn run(&self, _e: &(), _component_mgr: &mut WorldDocMgr) {}
}

impl ComponentHandler<Color, CreateEvent, WorldDocMgr> for BorderColorSet {
    fn handle(&self, event: &CreateEvent, component_mgr: &mut WorldDocMgr) {
...
impl ComponentHandler<Color, DeleteEvent, WorldDocMgr> for BorderColorSet {
    fn handle(&self, event: &DeleteEvent, component_mgr: &mut WorldDocMgr) {
        let DeleteEvent { id: _, parent } = event;
        let render_obj_id = component_mgr
            .node
...
impl RadiusSet {
    pub fn init(component_mgr: &mut WorldDocMgr) -> Rc<RadiusSet> {
        ...
        component_mgr
            ...
            ._group
            .register_modify_field_handler(Rc::downgrade(
                &(r.clone() as Rc<ComponentHandler<Rect, ModifyFieldEvent, WorldDocMgr>>),
            ));
        r
    }
}

impl System<(), WorldDocMgr> for RadiusSet {
    fn run(&self, _e: &(), _component_mgr: &mut WorldDocMgr) {}
}

impl ComponentHandler<Rect, CreateEvent, WorldDocMgr> for RadiusSet {
    fn handle(&self, event: &CreateEvent, component_mgr: &mut WorldDocMgr) {
...
impl ComponentHandler<Rect, ModifyFieldEvent, WorldDocMgr> for RadiusSet {
    fn handle(&self, event: &ModifyFieldEvent, component_mgr: &mut WorldDocMgr) {
        let ModifyFieldEvent {
            id,
            parent: _,
...
impl RectBind {
    pub fn new(id: usize, mgr: &mut WorldDocMgr) -> RectBind {
...
impl Bind for RectBind {
    unsafe fn init_locations(&self, context: usize, renderobj_id: usize) {
        let mgr = &mut *(context as *mut WorldDocMgr);
