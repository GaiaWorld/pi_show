...
use object2d::system::render_util::sdf::init_location;
use object2d::Object2dMgr;
use object2d::component::sdf::{SdfEffect, SdfDefines};

pub struct CreateSdfProgram(Rc<RefCell<DirtyMark>>);

impl CreateSdfProgram {
    pub fn init(component_mgr: &mut Object2dMgr) -> Rc<CreateSdfProgram>{
        let r = Rc::new(CreateSdfProgram(Rc::new(RefCell::new(DirtyMark::new()))));
        component_mgr.sdf_effect.defines._group.register_modify_field_handler(Rc::downgrade(&(r.clone() as Rc<ComponentHandler<SdfDefines, ModifyFieldEvent, Object2dMgr>>)));
        component_mgr.sdf_effect._group.register_delete_handler(Rc::downgrade(&(r.clone() as Rc<ComponentHandler<SdfEffect, DeleteEvent, Object2dMgr>>)));
        component_mgr.sdf_effect._group.register_create_handler(Rc::downgrade(&(r.clone() as Rc<ComponentHandler<SdfEffect, CreateEvent, Object2dMgr>>)));
        r
    }
}

impl ComponentHandler<SdfDefines, ModifyFieldEvent, Object2dMgr> for CreateSdfProgram{
    fn handle(&self, event: &ModifyFieldEvent, _component_mgr: &mut Object2dMgr){
        let ModifyFieldEvent{id: _, parent, field: _} = event;
        self.0.borrow_mut().marked_dirty(*parent);
    }
}

impl ComponentHandler<SdfEffect, DeleteEvent, Object2dMgr> for CreateSdfProgram{
    fn handle(&self, event: &DeleteEvent, _component_mgr: &mut Object2dMgr){
        let DeleteEvent{id, parent: _} = event;
        self.0.borrow_mut().delete_dirty(*id);
    }
}

impl ComponentHandler<SdfEffect, CreateEvent, Object2dMgr> for CreateSdfProgram{
    fn handle(&self, event: &CreateEvent, _component_mgr: &mut Object2dMgr){
        let CreateEvent{id, parent: _} = event;
        self.0.borrow_mut().marked_dirty(*id);
    }
}

impl System<(), Object2dMgr> for CreateSdfProgram{
    fn run(&self, _e: &(), component_mgr: &mut Object2dMgr){
