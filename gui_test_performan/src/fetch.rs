...
pub fn create_world<C: Context + Sync + Send + 'static>(mut engine: Engine<C>, width: f32, height: f32) -> World{
    ...
    world.register_system(LYOUT_N.clone(), CellLayoutSys::<YgNode>::new(LayoutSys::new()));
    world.register_system(TEXT_LAYOUT_N.clone(), CellLayoutImpl::new(LayoutImpl::<C, YgNode>::new()));
...
pub struct GuiWorld {
    ...
    pub yoga: Arc<CellMultiCase<Node, YgNode>>,
...
impl GuiWorld {
    pub fn new(world: World) -> GuiWorld{
        GuiWorld{
            ...
            yoga: world.fetch_multi::<Node, YgNode>().unwrap(),
...
fn create(world: &GuiWorld) -> usize {
    let idtree = world.idtree.lend_mut(); // 10000 节点， 64us
    let node = world.node.lend_mut().create(); // 10000 节点， 300us （一次hash表查询， 一次lend_mut, 一次slab插入）
    let border_radius = world.border_radius.lend_mut(); // 10000 节点， 90us
